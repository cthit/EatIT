...
export default class Swish extends Component {
    ...
    return (
      <form className="submit-swish" onSubmit={this.onSubmit}>
        ...
        <input
          ...
          name="swishNbr"
          onInput={e =>
            this.setState({ swishNbr: e.target.value.replace(/[^\d]/g, "") })
          }
          value={swishNbr}
  ...
  renderSwishInfo = () => {
    const { order: { swishNbr, swishName } } = this.props;
    const { showSwish } = this.state;
    return (
      <React.Fragment>
        <label>{swishName}</label>
        <label>{swishNbr}</label>
        <div className="button" id="swish-button" onClick={this.openSwish}>
          <img src="img/swish.png" alt="Swish" width="32" /> Tap to pay with
          Swish
        </div>
        {showSwish && (
          <React.Fragment>
            <p id="swish-notice">
              Link only works on mobile devices with the Swish app installed,
              alternatively you can scan this code with the Swish app:
            </p>
  ...
  render() {
    const { order: { swishNbr } } = this.props;
      ...
      <div className="timer-area container-part" id="swish-container">
        <div className="container-content">
          ...
          <div className="swish-box">
            {swishNbr ? this.renderSwishInfo() : this.renderSwishForm()}
          </div>
        </div>
      </div>
    );
